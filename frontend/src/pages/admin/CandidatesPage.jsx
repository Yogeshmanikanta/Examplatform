...
export default function CandidatesPage() {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        <input
          ...
          style={{
            ...
            fontSize: '13px', width: '280px', fontFamily: FONT, outline: 'none',
      ...
      {/* Table */}
      <div style={{ background: '#fff', borderRadius: '14px', border: '1px solid #f1f5f9', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px', color: '#94a3b8' }}>
            <div style={{ fontSize: '32px', marginBottom: '10px' }}>👥</div>
            <p style={{ fontSize: '14px' }}>{search ? 'No candidates match your search.' : 'No candidates registered yet.'}</p>
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #f1f5f9', background: '#f8fafc' }}>
                {['#', 'Name', 'Email', 'Mobile', 'Verified', 'Joined', isSuperAdmin ? 'Actions' : ''].filter(Boolean).map(h => (
                  <th key={h} style={{
            ...
            <tbody>
              {filtered.map((c, i) => (
                <tr key={c.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #f8fafc' : 'none', transition: 'background 0.1s' }}
                  onMouseEnter={e => e.currentTarget.style.background = '#fafafa'}
                  onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                >
                  <td style={{ padding: '14px 16px', fontSize: '13px', color: '#94a3b8', width: '40px' }}>{i + 1}</td>
                  <td style={{ padding: '14px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      <span style={{ fontSize: '13px', fontWeight: 600, color: '#0f172a' }}>{c.full_name}</span>
                    </div>
                  </td>
                  <td style={{ padding: '14px 16px', fontSize: '13px', color: '#475569' }}>{c.email}</td>
                  <td style={{ padding: '14px 16px', fontSize: '13px', color: '#475569' }}>{c.mobile || '—'}</td>
                  <td style={{ padding: '14px 16px' }}>
                    <span style={{
                      fontSize: '11px', fontWeight: 700, padding: '3px 10px', borderRadius: '20px',
                      background: c.is_verified ? '#ecfdf5' : '#fef2f2',
                      color: c.is_verified ? '#059669' : '#ef4444',
                    }}>
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        ...
                        <button
                          ...
                          style={{
                            padding: '6px 14px', borderRadius: '7px', border: '1px solid #fecaca',
        ...
        <Modal title="Edit Candidate" onClose={() => setEditModal(null)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '8px' }}>
              ...
              <button onClick={() => setEditModal(null)} style={btnSecondary}>Cancel</button>
            </div>
          </div>
        </Modal>
      )}
      ...
      {deleteModal && (
        <Modal title="Delete Candidate" onClose={() => setDeleteModal(null)}>
          <p style={{ fontSize: '14px', color: '#475569', marginBottom: '20px' }}>
            Are you sure you want to delete <strong style={{ color: '#0f172a' }}>{deleteModal.full_name}</strong>? This will permanently remove their account and all exam data.
          </p>
          <div style={{ display: 'flex', gap: '10px' }}>
            <button onClick={handleDelete} disabled={saving} style={{ ...btnPrimary, background: '#ef4444', boxShadow: 'none' }}>
...
function Field({ label, children }) {
  return (
    <div>
      <label style={{ fontSize: '12px', fontWeight: 700, color: '#64748b', display: 'block', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</label>
      {children}
    </div>
  );
}
