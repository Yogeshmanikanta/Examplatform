...
export const ExamModel = {
  ...
  async findAllForAdmin() {
    const result = await pool.query(`SELECT count(*) as total_exams FROM exams`);
    return result.rows[0];
  },
  ...
  async getStats(id) {
    const questions = await pool.query('SELECT COUNT(*) FROM questions WHERE exam_id=$1', [id]);
    const attempts = await pool.query('SELECT COUNT(*) FROM exam_attempts WHERE exam_id=$1', [id]);
    return {
